...
class usuarios{
    ...
    function changepassword($vs) {
        if ((isset($vs['changepassword_pass_old'])) && ($vs['changepassword_pass_old'] != '') && (isset($vs['changepassword_pass_new'])) && ($vs['changepassword_pass_new'] != '')) {
            $sql = sprintf("select * from usuario where usuario_id=" . $_SESSION['ctrlid'] . " AND usuario_pass='%s'", mysql_real_escape_string($vs['changepassword_pass_old']));
            $qs = mysql_query($sql);
            if (mysql_num_rows($qs) > 0) {
                $sqlc = sprintf("UPDATE usuario SET usuario_pass='" . $vs['changepassword_pass_new'] . "' WHERE usuario_id='%s'", mysql_real_escape_string($_SESSION['ctrlid']));
