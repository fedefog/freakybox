			        ...
			        <tr id="item_<?=$mr[$maintable."_id"]?>">
				        ...
				        foreach($_fields as $_field){?>
				            ...
				            foreach($doscampos as $campos){
				                if($fields[$_field]['fieldcomponent']=="lista"){
						            ...
						            echo $listasel;
                                    	...
                                    	elseif($fields[$_field]['fieldcomponent']=="imagen"){
				                			if(($mr[$_field]!='')&&(upload_exists($mr[$_field], 'x50y50'))){?>
                                            	<a href="<?=upload_path($mr[$_field]);?>" rel="shadowbox"><img src="<?=upload_path($mr[$_field], 'x50y50');?>" border="0" width="50" height="50" /></a>
                                            <?}
				                		}
				                    	...
				                    	$seteable=true;
                                                ...
                                                elseif($fields[$_field]['fieldcomponent']=="radio"){
                                                    if ($fields[$_field]['set_in_list'] == true || $fields[$_field]['set_in_list']==1){
                                                        printField($_field,$fields[$_field]['fieldcomponent'],$mr,$pathprocess,$fields[$_field]);
				                    	$seteable=true;
                                                    }
                                                    else{
								...
								elseif($fields[$_field]['fieldcomponent']=="url"){
									echo '<a href="'.$mr[$_field].'" target="_blank">'.$mr[$_field].'</a>';
								}
								elseif($fields[$_field]['fieldcomponent']=="email"){
									echo '<a href="mailto:'.$mr[$_field].'" target="_blank">'.$mr[$_field].'</a>';
								}
				                else{
