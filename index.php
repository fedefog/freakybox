...
if($uri->segment(1) == 'ajax'){
	...
	if($uri->segment(2) == 'complete'){
		...
		mysql_query("INSERT INTO actualizacion SET fk_tarea_id = '".intval($task_id)."', fk_proyecto_id = '".$task['fk_proyecto_id']."',fk_usuario_id = '$usuario_id', actualizacion_contenido = 'Completó esta tarea'");
		die("ok");
	}
	if($uri->segment(2) == 'incomplete'){
		...
		$task = getRow("SELECT fk_proyecto_id FROM tarea WHERE tarea_id = '".intval($task_id)."'");
		mysql_query("INSERT INTO actualizacion SET fk_tarea_id = '".intval($task_id)."', fk_proyecto_id = '".$task['fk_proyecto_id']."', fk_usuario_id = '$usuario_id', actualizacion_contenido = 'Marcó como incompleta esta tarea'");
		die("ok");
	}
